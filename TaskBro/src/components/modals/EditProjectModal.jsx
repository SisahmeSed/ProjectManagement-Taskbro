...
export default function EditProjectModal({ project, onClose, onSaved, onSuccess }) {
  const id = project.id ?? project.project_id
  const [name, setName]       = useState(project.project_name || project.name || "")
  ...
  const handleSave = async () => {
    ...
    try { await updateProject({ id, name: name.trim(), description: desc.trim() }); onSaved(); onSuccess?.(); onClose() }
    catch { setError("Could not update project.") }
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between">
        <div>
          <h2 className="text-[15px] font-bold text-gray-900 tracking-tight">Edit project</h2>
          <p className="text-xs text-gray-400 mt-0.5">Update project details</p>
        </div>
      ...
      {error && <div className="bg-red-50 border border-red-200 rounded-[10px] px-3.5 py-2.5 text-[13px] text-red-800">{error}</div>}
      ...
      <div className="flex gap-2 justify-end pt-1">
        ...
        <button onClick={handleSave} disabled={loading} className={`px-5 py-2 rounded-[9px] text-[13px] font-semibold text-white bg-gray-900 border-none shadow-sm transition-all ${loading ? "opacity-60 cursor-not-allowed" : "cursor-pointer hover:bg-gray-800"}`}>
